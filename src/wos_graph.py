...
def is_equal(A1, A2):
    """Checks for authors A1 and A2 to see if they can be considered the same.
    If one of the sets is a subset of the other, then
    we believe that the authors are equal.

    Args:
        A1 (list of strings): list with authors.
        A2 (list of strings): list with authors.
...
def del_equal_articles(articles):
    ...
    Args:
        articles(list): list with articles
...
def build_graph(articles, topic_name):
    """Builds a citation graph based on articles.
    In this function, article == vertex, link == oriented edge.

    Args:
        articles(list): list of articles.
        topic_name(str): name for the new graph.

    Returns:
        list: articles without duplicates.
    """
