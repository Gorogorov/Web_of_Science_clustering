...
# Удаляет статьи, у которых нет абстрактов
...
# Строит по статьям csv-файл и сохраняет его
...
def article_clasterization(topic_name, showtfidf, showmds,
    ...
    # Создаем tf-idf матрицу, подробнее на википедии.
    ...
    # dist - список расстояний, расстояние - 1 - евклидово расстояние в
    ...
    # distortins - сумма квадратов расстояний от каждого из центров
    ...
    for i in range(num_clusters):
        ...
        cnt_res_words = 0
        ind_res_words = 0
        # Выводим минимум между 20-ю и количеством топ-слов для
        # каждого кластера
        while (cnt_res_words <= 10 and
               ind_res_words < len(order_centroids[i])):
            if (vocab_frame.
               ix[terms[order_centroids[i][ind_res_words]].
                  split(' ')].
    ...
    # Если есть параметр showlds, обучаем lds модель (см. Википедию/
    # Хабр)
    if showlda:
    ...
    # Если есть параметр showmds, показываем mds (см. Википедию)
    if showmds:
    ...
    # Hierarchical Document Clustering (см. Википедию)
    if showhdc:
